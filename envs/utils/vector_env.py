...
PLAN_ACT_AND_PREPROCESS_TEMPLATE = "plan_act_and_preprocess_template"
...
LOAD_INITIAL_COMMAND_TEMPLATE = "load_initial_scene_template"
LOAD_NEXT_COMMAND = "load_next_scene"
LOAD_NEXT_COMMAND_TEMPLATE = "load_next_scene_template"
...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                elif command == LOAD_INITIAL_COMMAND_TEMPLATE:
                    obs, info, actions_dict = env.load_initial_scene_template()
                    connection_write_fn((obs, info, actions_dict))
                ...
                elif command == LOAD_NEXT_COMMAND_TEMPLATE:
                    obs, info, actions_dict = env.load_next_scene_template(data)
                    connection_write_fn((obs, info, actions_dict))
                ...
                # TODO: update CALL_COMMAND for getting attribute like this
                    ...
                    #if auto_reset_done and done:
                    #    observations, info = env.reset()
                    connection_write_fn((observations, reward, done, info, gs, nsd,planner_inputs ))
                    
                elif command == PLAN_ACT_AND_PREPROCESS_TEMPLATE:
                    observations, reward, done, info, gs, nsd  = \
                            env.plan_act_and_preprocess_template(data[0], data[1])
                    #if auto_reset_done and done:
                    #    observations, info = env.reset()
    ...
    def load_next_scene_template(self, load):
        ...
        #load should be a list even if it len(self._connection_write_fns) == 1
        for e, write_fn in enumerate(self._connection_write_fns):
            write_fn((LOAD_NEXT_COMMAND_TEMPLATE, load[e]))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, info, actions_dict = zip(*results)
    ...
    def load_initial_scene_template(self):
        self._is_waiting = True
        for write_fn in self._connection_write_fns:
            write_fn((LOAD_INITIAL_COMMAND_TEMPLATE, None))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, info, actions_dict = zip(*results)
    ...
    def plan_act_and_preprocess_template(self, inputs, goal_spotted):
        self._assert_not_closed()
        self._is_waiting = True
        for e, write_fn in enumerate(self._connection_write_fns):
            #if len(self._connection_write_fns) ==1:
            ...
            #else:
            data_list = [inputs[e], goal_spotted[e]]
            write_fn((PLAN_ACT_AND_PREPROCESS_TEMPLATE, data_list))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        #import pickle
