...
class VecPyTorch():
    ...
    def load_initial_scene_template(self):
        obs, info, actions_dict = self.venv.load_initial_scene_template()
        obs = torch.from_numpy(obs).float().to(self.device)
        return obs, info, actions_dict
    ...
    def load_next_scene_template(self, load):
        obs, info, actions_dict = self.venv.load_next_scene_template(load)
        obs = torch.from_numpy(obs).float().to(self.device)
        return obs, info, actions_dict
    ...
    #def plan_act_and_preprocess(self, inputs, goal_spotted):
        ...
        #else:
            #obs, reward, done, info, gs, next_step_dict, planner_inputs  = self.venv.plan_act_and_preprocess(inputs, goal_spotted)
        #obs = torch.from_numpy(obs).float().to(self.device)
        #reward = torch.from_numpy(reward).float()
    ...
    def plan_act_and_preprocess_template(self, inputs, goal_spotted):
        obs, reward, done, info, gs, next_step_dict  = self.venv.plan_act_and_preprocess_template(inputs, goal_spotted)
        obs = torch.from_numpy(obs).float().to(self.device)
        reward = torch.from_numpy(reward).float()
        #return obs, reward, done, info, gs[0], next_step_dict[0]
        return obs, reward, done, info, gs, next_step_dict
...
###################################################################
    # files_sorted = []
    # test_trials = [
    #     ['trial_T20190907_102345_652342', '0'],      # pickup same sliced instance
    #     ['trial_T20190906_230538_685342', '0']       # potato slice / pickup egg ???
    # ]
    # for i in files:
    #     trial, repeat_idx, task_type = i['task'].split('/')[-1], i['repeat_idx'], i['task'].split('-')[0]

    #     if [trial, str(repeat_idx)] in test_trials:                   # tests_seen/unseen

    #         files_sorted.append(i)
