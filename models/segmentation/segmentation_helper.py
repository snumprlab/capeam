...
class SemgnetationHelper:
	...
	def sem_seg_get_put_recep_under_obj(self, obj_mask):
		...
		dilation_obj_mask  = cv2.dilate(obj_mask, kernel, iterations=1)

		#Get the highest score
		for i in range(large_len):
			
			v = self.segmented_dict['large']['masks'][i]
			score = np.sum(dilation_obj_mask*v)
	...
	def sem_seg_get_recepmask_under_obj(self, obj_mask):
		...
		for i in range(large_len):
			...
			if score>max_score :
				mask = self.segmented_dict['large']['masks'][i]
				max_score = score
