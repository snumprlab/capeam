...
from models.instructions_processed_LP.ALFRED_task_helper import get_list_of_highlevel_actions, determine_consecutive_interx, get_arguments, get_arguments_test, read_test_dict , get_list_of_highlevel_actions_template
...
class Sem_Exp_Env_Agent_Thor(ThorEnvCode):
	...
	def __init__(self, args, scene_names, rank):
		...
		self.test_dict = read_test_dict(self.args.test, self.args.appended, 'unseen' in self.args.eval_split, self.args.templated)
	...
	def load_initial_scene_template(self):
		self.side_step_order = 0
		self.rotate_before_side_step_count = 0
		self.fails_cur = 0
		self.put_rgb_mask = None
		...
		try:
			...
			if self.args.test == False and not(self.args.valid_learned_lang):
				...
				list_of_actions, categories_in_inst, second_object, caution_pointers = get_list_of_highlevel_actions_template(traj_data, args_nonsliced = self.args.nonsliced)
			else:
				instruction = traj_data['turk_annotations']['anns'][r_idx]['task_desc']
				...
				list_of_actions, categories_in_inst, second_object, caution_pointers = get_list_of_highlevel_actions_template(traj_data, True, self.test_dict, self.args.nonsliced, self.args.appended)
	...
	def load_next_scene_template(self, load):
		if load == True:
			self.scene_pointer += 1
			obs, info, actions_dict = self.load_initial_scene_template()
			return obs, info, actions_dict
	...
	def reset_goal(self, truefalse, goal_name, consecutive_interaction):
		if truefalse == True:
			...
			if self.args.templated:
				self.goal_idx = self.total_cat2idx[goal_name]
			else:
				if goal_name in self.total_cat2idx:
					self.goal_idx = self.total_cat2idx[goal_name]
				else :
					self.goal_idx = self.total_cat2idx['None']
	...
	def whether_center(self):
		...
		if np.sum(self.interaction_mask) == 0: 
			return False
	...
	def plan_act_and_preprocess(self, planner_inputs, goal_spotted):
		...
		if self.side_step_order in [1,2]:
		...
		elif self.lookDownUpLeft_count in range(1,3):
			...
			if self.prev_number_action == 0:
				...
				elif self.is_visible_from_mask(self.interaction_mask, stricer_visibility_dist=self.args.stricter_visibility): 
					self.update_loc(planner_inputs)
					wd = self.which_direction()
					if self.args.debug_local:
			...
			if self.args.use_sem_seg and self.flag== None:
				self.print_log("obj type for mask is :", self.goal_idx2cat[self.goal_idx])
				self.interaction_mask = self.seg.sem_seg_get_instance_mask_from_obj_type(self.goal_idx2cat[self.goal_idx])
			elif self.args.use_sem_seg and self.flag!= None:
				self.print_log("obj type for mask is :", self.goal_idx2cat[self.goal_idx])
				self.interaction_mask = None
				self.flag= None
			else:
			...
			if pointer in self.convert_pointers :
				small_target_mask=self.seg.sem_seg_get_instance_mask_from_obj_type(list_of_actions[pointer][2])
				visible = self.is_visible_from_mask(small_target_mask, stricer_visibility_dist=self.args.stricter_visibility)
				planner_inputs['sem_map_pred']
    
				if visible or np.sum(planner_inputs['class_map'][self.total_cat2idx[list_of_actions[pointer][2]]])>0: 
					self.execute_interaction = visible
					self.goal_idx = self.total_cat2idx[list_of_actions[pointer][2]]

					planner_inputs['list_of_actions'][pointer]=planner_inputs['list_of_actions'][pointer+1]
					planner_inputs['list_of_actions'][pointer][3]='0'

					num_high_idx = 0
					for _,_ ,_,_, high_idx in planner_inputs['list_of_actions']:
						if high_idx ==planner_inputs['list_of_actions'][pointer][4] :
							num_high_idx+=1 
					for _ in range(num_high_idx-1) :
						del planner_inputs['list_of_actions'][pointer+1] 



					del self.convert_pointers[0]
					self.caution_pointers.remove(pointer)

					if len(self.convert_pointers) >0 :
						for convert_inx in range(len(self.convert_pointers)) :
							self.convert_pointers[convert_inx] -=(num_high_idx-1)

					if len(self.caution_pointers) >0 :
						for caution_inx in range(len(self.caution_pointers)) :
							self.caution_pointers[caution_inx] -=(num_high_idx-1)

					self.interaction_mask =small_target_mask
					print("----- Change action sequence---")
					for action_len in range(len(planner_inputs['list_of_actions'])) :
						print(planner_inputs['list_of_actions'][action_len])
					print("-----------------------")
		...
		return obs, rew, done, info, goal_success, next_step_dict,planner_inputs
	...
	def plan_act_and_preprocess_template(self, planner_inputs, goal_spotted):
		self.fails_cur = 0
		self.pointer = planner_inputs['list_of_actions_pointer']
	...
	def _get_stg(self, grid, start, goal, planning_window, found_goal, xy_forward, step, explored, goal_found, newly_goal_set, pointer):
		...
		if self.args.templated:
  		...
  		# remember sliced
			...
			if 'Sliced' in self.goal_name and np.sum(self.loc_whenSliced) != None :
				goal[self.loc_whenSliced[0]+1][self.loc_whenSliced[1]+1] = 1  
