...
def main():
    ...
    if args.templated:
        obs, infos, actions_dicts = envs.load_initial_scene_template()
    else:
    ...
    if args.templated:
        obs, rew, done, infos, goal_success_s, next_step_dict_s = envs.plan_act_and_preprocess_template(planner_inputs, goal_spotted_s)
    else:
        ...
        for e,x in enumerate(task_finish):
            if x:
                ...
                if not(finished[e]):
                    ...
                    if args.templated:
                        obs, infos, actions_dicts = envs.load_next_scene_template(load)
                    else:
            ...
            if args.delete_from_map_after_move_until_visible and (next_step_dict_s[e]['move_until_visible_cycled'] or next_step_dict_s[e]['delete_lamp']):
                ...
                for lab in unique_labels:
                    ...
                    if args.templated:
                        ...
                        center = (int(np.nanmean(wheres[0])), int(np.nanmean(wheres[1])))
